...
- [x] Railway 배포 로그/대시보드에서 최신 커밋이 실제로 배포됐는지 확인 (2026-07-05, `railway up`으로 배포, 배포 ID `c8b6ef51` SUCCESS)
- [x] 배포 후 `curl -sI https://bungaego.com/`의 `last-modified`가 갱신됐는지 재확인
- [x] 로그인 세션 유지 확인 (`/app-auth`로 대체 검증 — 실제 카카오 콘센트는 브라우저 필요)

### A-5. 사고 기록: 예약 생성이 배포 직후 전부 막혀 있었음 (2026-07-05)

**증상**: Stage A(0007) 적용 + 배포 완료 후 스모크 테스트로 `reservations.create`를 호출하니
`INTERNAL_SERVER_ERROR`. 실제 고객 예약도 전부 막혀 있었을 것으로 추정(발견 즉시 수정).

**원인**: `reservations.totalAmount`가 프로덕션에 `int NOT NULL`로 남아있는데 **DEFAULT 값이
없었음**. 결제 라인아이템 리팩터링(P3) 이후 새 코드는 이 컬럼에 값을 넣지 않고 `payments`
테이블에서 금액을 관리하는데, DB는 여전히 "DEFAULT 없는 NOT NULL"이라 INSERT 자체가 거부됨.
`status`/`paymentId`/`paymentMethod`/`cancelledAt`/`cancelReason` 5개는 전부 DEFAULT가 있거나
nullable이라 문제없었음 — `totalAmount` 하나만 문제였음.

**교훈 (Stage B/0006 적용 전까지, 그리고 앞으로 비슷한 expand 작업 때마다 기억할 것)**:

> **expand 단계에서 "신코드가 더 이상 안 쓰는 구 컬럼"을 당장 DROP하지 않고 남겨둘 때는,
> 그 컬럼이 `NOT NULL`이면서 `DEFAULT`가 없는지 반드시 확인하고, 없으면 DEFAULT를
> 부여해야 한다.** 그렇지 않으면 신코드의 INSERT(그 컬럼을 명시하지 않는 INSERT)가
> 전부 실패한다. DROP은 나중으로 미뤄도 되지만, "값 없이 INSERT 가능하게 만드는 것"은
> expand 단계에서 바로 해야 하는 필수 작업이다.

**적용한 수정**:
```sql
ALTER TABLE `reservations` ALTER COLUMN `totalAmount` SET DEFAULT 0;
```
DROP이 아니라 DEFAULT만 추가하는 것이라 0006(컬럼 삭제)과는 무관하고, 필요하면
`ALTER TABLE reservations ALTER COLUMN totalAmount DROP DEFAULT`로 즉시 원복 가능.

**검증**: 수정 후 실제 `reservations.create`/`reservations.cancel` API를 프로덕션에서
호출해 `reservations`(totalAmount=0) + `payments`(status=paid, totalAmount=실제금액) +
`payment_items`(type=fare) 행이 전부 정상 생성/취소되고 `trips.currentCount`가
원상복구되는 것까지 확인. 전체 핵심 테이블(reservations/payments/payment_items/
consents/users/trips/events/boarding_points)에 같은 패턴("DEFAULT 없는 NOT NULL을
신코드가 안 채우는 경우")이 더 있는지 `information_schema.columns` 기준으로 재점검—
나머지는 전부 신코드가 항상 명시적으로 채우는 진짜 필수 필드라 문제없음을 확인.
...
- `.env.production.local`은 gitignore 대상이며 이 문서 작성 시점에 커밋 스테이징에
  포함되지 않은 것을 확인했습니다.
- **A-5 사고 참고**: `reservations.totalAmount DEFAULT 0`은 임시 호환 조치입니다. Stage B(0006)에서
  이 컬럼 자체가 DROP되면 이 DEFAULT는 자연히 무의미해지므로 별도로 되돌릴 필요는 없습니다.
